 ...
 * Text Domain: ajax-cart-autoupdate-for-woocommerce
...
class acau_settings {
	...
	public function __construct() {
		...
		$this->settings_list = [
			...
			'acau_update_delay' 		=> [ 'val' => 1000, 	'title' => 'Update delay', 			'type' => 'number', 	'tab' => 'configuration',
											 'descr' => 'Time in miliseconds between the last quantity change and sending a request to update quantity on server (a spinning wheel is displayed when this happens). Allows changing the quantity multiple times with only one server request done in the end. Low values will result in the spinning wheel showing more than once when the changes are slow. High values will make the customers wait and wonder why the cart doesn\'t recalculate. How long the spinning wheel is displayed, is based entirely on the server response time.' ],
			'acau_positive_qty' 		=> [ 'val' => true, 	'title' => 'Cart minimum quantity', 'type' => 'checkbox', 	'tab' => 'configuration',
											 'descr' => 'Change minimum product quantity on cart page from 0 to 1. If unchecked, 0 quantity is valid and such products are removed on cart update.' ],
			'acau_cart_notices_off' 	=> [ 'val' => true, 	'title' => 'Cart page notices', 	'type' => 'checkbox', 	'tab' => 'configuration',
											 'descr' => 'Turn off notices on cart page, including "Cart updated." and "&lt;Product&gt; removed. Undo?". Empty cart message will still be displayed.' ],		
		];
	}
	...
	public function display_options() {
		...
		foreach($arr as $key => $item) {
			if ( $active_tab == $arr[$key]['tab']  ) {
				...
				add_settings_field (
					$key, // ID
					__( $arr[$key]['title'], 'ajax-cart-autoupdate-for-woocommerce' ), // Title
     ...
     */
		...
		switch ( $this->settings_list[$field]['type'] ) {
			...
			case 'checkbox' :
				...
				$info = isset ( $this->settings_list[$field]['info'] ) ? __( $this->settings_list[$field]['info'], 'ajax-cart-autoupdate-for-woocommerce' ) : '';

				printf (
					$fieldset,
					esc_attr( $field ),
					isset( $this->options[$field] ) && ( 1 == $this->options[$field] )  ? 'checked="checked" ':'',
					__( $this->settings_list[$field]['descr'], 'ajax-cart-autoupdate-for-woocommerce' ),
					$info
				);			
				break;
			...
			case 'number' :
				...
				$descr = isset ( $this->settings_list[$field]['descr'] ) ? __( $this->settings_list[$field]['descr'], 'ajax-cart-autoupdate-for-woocommerce' ) : '';
...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) || ( get_site_option('active_sitewide_plugins') && array_key_exists( 'woocommerce/woocommerce.php', get_site_option('active_sitewide_plugins') ) ) ) {
	...
	function acau_hide_update_cart_button() {
